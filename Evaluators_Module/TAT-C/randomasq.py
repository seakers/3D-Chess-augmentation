...
#     if fov.get("sensorGeometry") == "CIRCULAR":
#         field_of_view = CircularGeometry(diameter=fov.get("fullConeAngle"))
#     elif fov.get("sensorGeometry") == "RECTANGULAR":
#         field_of_view=RectangularGeometry(
#         angle_height=fov.get("alongTrackFieldOfView") if fov.get("alongTrackFieldOfView") !=0 else fov.get("crossTrackFieldOfView"),
...
#     if np.issubdtype(observations['revisit'].dtype, np.timedelta64):
#         observations['revisit'] = observations['revisit'].dt.total_seconds()
#     elif np.issubdtype(observations['revisit'].dtype, np.datetime64):
#         observations['revisit'] = (observations['rvisit'] - observations['revisit'].min()).dt.total_seconds()
#     else:
#         print("⚠️ Unknown revisit dtype:", observations['revisit'].dtype)
#         observations['revisit'] = np.nan
