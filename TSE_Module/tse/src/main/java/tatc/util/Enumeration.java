...
public class Enumeration {
     ...
     */
    public static ArrayList<ConstellationParameters> fullFactHomogeneousWalker(List<Double> alts, List<Object> incs, List<Integer> ts, List<Integer> ps, List<Integer> fs, List<Satellite> sats, double eccentricity) throws IllegalArgumentException{
        ArrayList<ConstellationParameters> constels = new ArrayList<>();

        ts.sort(Integer::compareTo);
        if (ts.contains(0)) {
            constels.add(new HomogeneousWalkerParameters(0, 0, 0, 0, 0, null));
            ts.remove(0);
        }
        if (ps == null){
                            ...
                            if (fs.contains(i_f)){
                                for (Satellite sat : sats){
                                    if (archCounter <= maxNumberOfArchitecturesFF){
                                        constels.add(new HomogeneousWalkerParameters(altitude, inclinationModified, t, plane, i_f, sat));
                                        archCounter++;
                                    }else{
                ...
                for(Integer t : ts) {
                    ...
                    for(Integer plane : planes) {
                        for(int i_f = 0; i_f <= plane - 1; ++i_f) {
                            if (fs.contains(i_f)) {
                                if (archCounter <= maxNumberOfArchitecturesFF){
                                    constels.add(new HomogeneousWalkerParameters(altitude, inclinationModified, t, plane, i_f, null));
                                    archCounter++;
                                }else{
                                    System.out.println("Aborting TAT-C... This design space contains too many architectures" +
                                            " to run a full factorial enumeration. Consider using MOEA or KDO search strategies.");
